...
impl SessionInner {
    ...
    fn load_all_from_db(&mut self) -> Result<()> {
        ...
        let mut stmt = conn
            .prepare("SELECT id, title, messages_json, updated_at FROM sessions")
        ...
        let rows = stmt
            .query_map([], |row| {
                ...
                Ok((id, title, messages_json, updated_at))
            })
            .context("读取 sessions 行失败")?;

        self.sessions.clear();
        for row in rows {
            let (id, title, messages_json, updated_at) = row.context("解析 sessions 行失败")?;
            let messages: Vec<Message> = serde_json::from_str(&messages_json)
                .with_context(|| format!("解析 session messages 失败: {}", id))?;
            self.sessions.insert(
                id.clone(),
                CachedSession {
                    session: ChatSession { id, title, messages },
                    updated_at,
                },
            );
        }
    ...
    fn flush_to_db(&mut self) -> Result<()> {
        ...
        for id in self.dirty_sessions.clone() {
            if let Some(cached) = self.sessions.get(&id) {
                ...
                tx.execute(
                    "
                    INSERT INTO sessions(id, title, messages_json, updated_at)
                    VALUES(?1, ?2, ?3, ?4)
