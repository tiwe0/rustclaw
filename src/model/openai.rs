...
impl ChatModel for OpenAIModel {
    async fn chat_once(
        ...
        if response.status().as_u16() != 200 {
            ...
            log::error(format!(
                "[model/openai] chat_once non-200: status={} model={} base_url={} message_count={} tool_count={} body={}",
                status,
                self.model,
                self.base_url,
                messages.len(),
            ...
            return Err(anyhow!("OpenAI 请求返回错误状态: {}", status));
        }
    ...
    async fn stream_chat_collect(
        ...
        if response.status().as_u16() != 200 {
            ...
            let body = response.text().await.unwrap_or_else(|_| "<failed to read body>".to_string());
            log::error(format!(
                "[model/openai] stream_chat_collect non-200: status={} model={} base_url={} message_count={} tool_count={} body={}",
                status,
                self.model,
                self.base_url,
                messages.len(),
            ...
            return Err(anyhow!("OpenAI 流式请求返回错误状态: {}", status));
        }
